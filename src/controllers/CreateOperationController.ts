import {
  ICreateOperationDTO,
  operationType,
} from "@repositories/DTO/ICreateOperationDTO";
...
class CreateOperationController {
  async handler(event: any): Promise<any> {
    try {
      const { balanceMoved, cpf, typeOperation } = JSON.parse(
        event.body
      ) as ICreateOperationDTO;

      const { error } = schemaCreateOperation.validate({
        cpf: cpf,
      ...
      const response = await this.factoryOperation(
        typeOperation,
        cpfvalidado,
        balanceMoved
      );
  ...
  private async factoryOperation(
    typeOperation: string,
    cpf: string,
    balanceMoved: number
  ): Promise<ReturnOperation> {
    switch (typeOperation) {
      case operationType.deposit:
        const responseDeposit = container.resolve(
          CreateOperationDepositUseCase
        );
        return responseDeposit.execute(balanceMoved, cpf);
      case operationType.withdraw:
        const responseWithdraw = container.resolve(
          CreateOperationWithdrawUseCase
        );
