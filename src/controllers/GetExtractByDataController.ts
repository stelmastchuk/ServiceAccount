...
class GetExtractByDataController {
  async handler(event: any): Promise<any> {
    try {
      ...
      const { cpf, startDate, endDate } =
        queryStringParameters as IGetExtractByDataDTO;

      const { error } = schemaGetExtract.validate({
        cpf: cpf,
        startDate: startDate,
        endDate: endDate,
      });
      ...
      const getExtractByDataUseCase = container.resolve(
        GetExtractByDataUseCase
      );

      const response = await getExtractByDataUseCase.execute(
        cpfvalidado,
        startDate,
        endDate
      );
