...
import { IBlockOrDesblockAccountDTO } from "@repositories/DTO/IBlockOrDesblockAccountDTO";
...
class BlockOrUnlockAccountController {
  async handler(event: any): Promise<any> {
    try {
      const { cpf, accountStatus } = JSON.parse(
        event.body
      ) as IBlockOrDesblockAccountDTO;

      const { error } = schemaBlockOrDesblockAccount.validate({
      ...
      const blockOrDesblockAccountUseCase = container.resolve(
        BlockOrUnlockAccountUseCase
      );

      const response = await blockOrDesblockAccountUseCase.execute(
        cpfvalidado,
        accountStatus
      );
      ...
      return {
        statusCode: 200,
        body: JSON.stringify({
          account: response ? "account unlocked" : "account blocked",
        }),
