...
describe("Create Operation Withdraw", () => {
    ...
    const createHistoricAccountRepo: jest.Mocked<IHistoricAccountRepository> = {
        ...
        getAccountHistoricPerDay: jest.fn().mockImplementation(() => Promise.resolve(historicMock))
    }
    ...
    it("should not be able to deposit in Account, if account Insufficient balance  ", async () => {
        ...
        const usecase = new CreateOperationWithdrawUseCase(createAccountRepo, createHistoricAccountRepo)
        const mockError = new AppError("Insufficient balance to complete the transaction!")
        await (expect(usecase.execute(1000, "42845684002"))).rejects.toEqual(mockError)
    });
