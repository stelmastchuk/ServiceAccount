...
class HistoricAccountRepository implements IHistoricAccountRepository {
  ...
  async getAccountHistoricByData(
    cpf: string,
    startDate: string,
    endDate: string
  ): Promise<Partial<HistoricAccount[]>> {
    const startDateTimeStamp =
      DateFns.startOfDay(new Date(startDate).getTime()).getTime() -
      Datetimezone.getTimezoneOffset("America/Sao_Paulo");
    const endDateTimeStamp =
      DateFns.endOfDay(new Date(endDate).getTime()).getTime() -
      Datetimezone.getTimezoneOffset("America/Sao_Paulo");

    const historic: any = await HistoricAccountRepositoryModel.query("cpf")
      .eq(cpf)
      .filter("dateCreation")
      .ge(startDateTimeStamp)
      .filter("dateCreation")
      .le(endDateTimeStamp)
      .exec();
  ...
  async getAccountHistoricPerDay(
    cpf: string,
    startDate: number,
    endDate: number
  ): Promise<Partial<HistoricAccount[]>> {
